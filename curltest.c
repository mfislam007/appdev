...
int main(int argc, char **argv){
	...
	if(argc < 3){
		...
		return 1;	//program stops because theuser has not given enough info
	}
	...
	c = curl_easy_init();
	if(c){
		curl_easy_setopt(c,CURLOPT_URL, "http://www.cc.puv.fi/~e1701764/curl.php");
		curl_easy_setopt(c,CURLOPT_POSTFIELDS, poststr);
		res = curl_easy_perform(c);
		if(res !=CURLE_OK)
		printf("Error in curl_easy_perform(c).\n");
		else
